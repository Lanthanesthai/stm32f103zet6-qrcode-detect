                                ...
                                int verbosity)
{
    ...
    err->buf = Zbar_realloc(err->buf, len);
...
//                err->arg_str = strdup("<?>");
    ...
    if(err->type == ZBAR_ERR_SYSTEM) {
        ...
        err->buf = Zbar_realloc(err->buf, len + strlen(sysfmt) + strlen(syserr));
        ...
        err->buf = Zbar_realloc(err->buf, len + 2);
