...
zbar_decoder_t *zbar_decoder_create ()
{
    zbar_decoder_t *dcode = Zbar_calloc(1, sizeof(zbar_decoder_t));
	   if(!dcode){
			printf("calloc dcode object faild\n ");
		}
    dcode->buf_alloc = BUFFER_MIN;
    dcode->buf =  Zbar_malloc(dcode->buf_alloc);
		if(!dcode->buf){
			printf("malloc dcode->buf object faild\n ");
		}
...
{
     Zbar_memset(dcode, 0, (long)&dcode->buf_alloc - (long)dcode);
#ifdef ENABLE_EAN
    ean_reset(&dcode->ean);
#endif
#ifdef ENABLE_I25
...
{
    /* soft reset decoder */
    Zbar_memset(dcode->w, 0, sizeof(dcode->w));
    dcode->lock = 0;
    dcode->idx = 0;
#ifdef ENABLE_EAN
