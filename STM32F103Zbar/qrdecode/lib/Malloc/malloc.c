...
//c :要设置的值
//num:需要设置的内存大小(字节为单位)
void mymemset(void *s,u8 c,u32 num)  
{  
    u8 *xs = s;  
    while(num--)*xs++=c;  
}	   
...
{  
    mymemset(malloc_cortol.memmap[memx], 0,memtblsize[memx]*2);//内存状态表数据清零  
	mymemset(malloc_cortol.membase[memx], 0,memsize[memx]);	//内存池所有数据清零  
...
{  
    ...
    {  									   
	    mymemcpy((void*)((u32)malloc_cortol.membase[memx]+offset),paddr,size);	//拷贝旧内存内容到新内存   
